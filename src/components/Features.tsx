...
const FeatureCard: React.FC<FeatureCardProps> = ({ icon, title, description, delay }) => {
  ...
  return (
    <div 
      ref={cardRef}
      className={cn(
        "glass-card p-8 text-center transition-all duration-700 ease-out opacity-0 translate-y-10",
        "hover:shadow-lg hover:scale-[1.02] transition-transform duration-300"
      )}
    >
      <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-kuma-light/30 text-kuma-primary mb-6 mx-auto">
        {icon}
      </div>
      <h3 className="text-xl font-bold mb-3 text-kuma-primary">{title}</h3>
  ...
  return (
    <section id="features" className="py-24 relative">
      <div className="max-w-7xl mx-auto px-6">
        <h2 
          ref={titleRef}
          className="text-3xl md:text-4xl font-bold text-center mb-16 text-kuma-primary transition-all duration-700 ease-out opacity-0 translate-y-10"
        >
          How Kuma Works
        </h2>
